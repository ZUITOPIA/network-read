...
인터넷과 WAN의 주된 차이점 : 네트워크 관리자, 통신 상대

1) 네트워크 관리자 기준
  - WAN : 전기통신사업자가 모두 관리
  - 인터넷 : 전기통신사업자가 모두 관리 X, 접속사업자인 ISP들이 각각 분담하여 자신들의 네트워크를 관리함
...
```
데이터를 주고받기 위해서 필요한 다양한 규약
 - 데이터의 포맷, 데이터 송수신 순서, 데이터의 송신처, 에러처리
...
```
통신에 필요한 기능들을 모듈화 해 계층 구조로 만든 후, 각 계층의 기능을 구현하기 위한 통신 프로토콜을 조합함
  ...
  - 국제표준화기구 ISO(International Organization for Standardization)가 제정한 것
	7가지 계층으로 구분 (복잡하기 때문에 일반적으로 이용되고 있지는 않음)
        ...
        - 네트워크 계층
 		- 데이터링크 계층
  		- 물리 계층 (최하위)
  - 주로 네트워크 상태에 대해 언급할 때의 잣대로 사용됨
  (ex. 장애 발생 시 '이건 물리 계층의 문제다!'와 같은 말)
```
...
1) TCP/IP 규정으로 구분하기 - 응용, 전송, 인터넷 계층 vs 네트워크 인터페이스 계층
  ...
  - 네트워크 인터페이스 계층 : TCP/IP에서 딱히 규정하고 있지 않음
   (주로 LAN이나 WAN에서의 데이터를 전송하기 위한 것으로 네트워크 구성에 의존하지 않고
    어떠한 LAN, WAN 네트워크라도 TCP/IP 통신을 구현할 수 있음)
